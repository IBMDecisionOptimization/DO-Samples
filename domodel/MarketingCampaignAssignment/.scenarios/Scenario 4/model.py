...
list_of_Customer = list_of_Customer[['age', 'id']].copy()
list_of_Customer.rename(columns={'age': 'age', 'id': 'id'}, inplace=True)
...
def build_model():
    ...
    join_Customer_ResourceAssignment = list_of_Customer.join(list_of_ResourceAssignment, how='inner')
    groupbyLevels = [join_Customer_ResourceAssignment.index.names.index(name) for name in list_of_Customer.index.names]
    groupby_Customer_ResourceAssignment = join_Customer_ResourceAssignment.resourceAssignmentVar.groupby(level=groupbyLevels).sum().to_frame()
    for row in groupby_Customer_ResourceAssignment.itertuples(index=True):
    ...
    # [ST_3] Constraint : cResourceRelationalConstraint_cIterativeRelationalConstraint
    # For each Customer where age is less than 20, number of assigned Campaigns where Campaign is Home is equal to 0
    # Label: CT_3_For_each_Customer_where_age_is_less_than_20__number_of_assigned_Campaigns_where_Campaign_is_Home_is_equal_to_0
    filtered_Customer = list_of_Customer[list_of_Customer.age < 20].copy()
    join_Customer_ResourceAssignment = filtered_Customer.join(list_of_ResourceAssignment, how='inner')
    join_Customer_ResourceAssignment_Customer_ResourceAssignment = join_Customer_ResourceAssignment.join(join_Customer_ResourceAssignment, rsuffix='_right', how='inner')
    filtered_Customer_ResourceAssignment_Customer_ResourceAssignment = join_Customer_ResourceAssignment_Customer_ResourceAssignment.loc[helper_get_level_values(join_Customer_ResourceAssignment_Customer_ResourceAssignment, 'id_of_Campaign') == 'Home'].copy()
    groupbyLevels = [filtered_Customer_ResourceAssignment_Customer_ResourceAssignment.index.names.index(name) for name in list_of_Customer.index.names]
    groupby_Customer_ResourceAssignment_Customer_ResourceAssignment = filtered_Customer_ResourceAssignment_Customer_ResourceAssignment.resourceAssignmentVar.groupby(level=groupbyLevels).sum().to_frame()
    for row in groupby_Customer_ResourceAssignment_Customer_ResourceAssignment.itertuples(index=True):
        helper_add_labeled_cplex_constraint(mdl, row.resourceAssignmentVar == 0, 'For each Customer where age is less than 20, number of assigned Campaigns where Campaign is Home is equal to 0', row)
    
    # [ST_4] Constraint : cManualAssignment_cCategoryCompatibilityConstraintOnPair
    # Home is assigned to Customer 1
    # Label: CT_4_Home_is_assigned_to_Customer_1
    filtered_Customer = list_of_Customer.loc[['Customer 1']]
    join_Customer_ResourceAssignment = filtered_Customer.join(list_of_ResourceAssignment, how='inner')
    filtered_Campaign = list_of_Campaign.loc[['Home']]
    join_Customer_ResourceAssignment_Campaign = join_Customer_ResourceAssignment.reset_index().join(filtered_Campaign, on=['id_of_Campaign'], how='inner').set_index(join_Customer_ResourceAssignment.index.names)
    helper_add_labeled_cplex_constraint(mdl, mdl.sum(join_Customer_ResourceAssignment_Campaign.resourceAssignmentVar) >= 1, 'Home is assigned to Customer 1')
