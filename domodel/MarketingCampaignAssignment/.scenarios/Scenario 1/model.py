...
# Data collection: list_of_Campaign ['id']
...
list_of_Campaign = pd.DataFrame(inputs['Candidates']['Campaign'].unique(), columns=['id']).dropna()
...
# --- Handling table for implicit concept
list_of_Customer = pd.DataFrame(inputs['Candidates']['Customer'].unique(), columns=['id']).dropna()
...
def build_model():
    ...
    # The number of Customer assignments for each Campaign is less than or equal to 3
    # Label: CT_1_The_number_of_Customer_assignments_for_each_Campaign_is_less_than_or_equal_to_3
    groupbyLevels = [list_of_ResourceAssignment.index.names.index(name) for name in list_of_Campaign.index.names]
    ...
    for row in groupby_ResourceAssignment.itertuples(index=True):
        helper_add_labeled_cplex_constraint(mdl, row.resourceAssignmentVar <= 3, 'The number of Customer assignments for each Campaign is less than or equal to 3', row)
